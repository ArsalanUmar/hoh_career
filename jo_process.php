...
use PHPMailer\PHPMailer\SMTP;
  ...
  use \setasign\Fpdi\Fpdi;
    ...
    if(!empty($_POST)){
      ...
      $post = $_POST;
      $orientation_topics_file = 'pdf/Orientation Topics Covered.pdf';

       $name=       $file_name_pdf =  "";

      $organization = $record_details['organization'];
        $position = $record_details['position'];
        // Normalize organization: if removed/missing, default to Home Health to keep flow working
      ...
      if($organization == 'Both'){
            ...
            // Ensure base template for Medical Social Worker under Both
            if($position == 'Medical Social Worker' && empty($jo_pdf_file)){
              $jo_pdf_file = 'pdf/jo_medical_social_worker.pdf';
            }
            else if($position == 'Medical Social Worker'){
...
// echo $jo_pdf_file;die();
     ...
     // die();
       ...
       $pdf_orientation_topics_file_name = "JO_ORIENTATION_TOPICS_".$first_name." ".$last_name.' '.date('Y-m-d').'.pdf';
       ...
       $pdf_agr_file_name = "pdf_files/AGR_".$file_name_pdf.' '.date('Y-m-d').'.pdf';
        ...
        $file_name_pdf_2 = $file_name_pdf_3 = NULL;
         ...
         save_pdf($html,$mySignature, $mySignature, $mySignature,$pdf_jo_file_name,$pdf_jo_file_name_2 ,$pdf_jo_file_name_3, $pdf_jo_file_name_4,$pdf_agr_file_name,$jo_pdf_file,$jo_pdf_file_2,$jo_pdf_file_3,$jo_pdf_file_4,$file_name_pdf,$file_name_pdf_2,$file_name_pdf_3,$post,$orientation_topics_file,$pdf_orientation_topics_file_name);

          // send_mail($file_name,"flippincrazyllc@yahoo.com",$name,$date,$_POST);

         // echo $pdf_jo_file_name;die();

          $send_mail =send_mail($file_name,$email,$name,$date,$post,false,$pdf_jo_file_name,$pdf_jo_file_name_2,$pdf_jo_file_name_3, $pdf_jo_file_name_4, $pdf_agr_file_name,$file_name_pdf,$file_name_pdf_2,$file_name_pdf_3,$pdf_orientation_topics_file_name);
  ...
  /** Employee handbook PDF — place file in pdf/ folder (see jo_send_handbook_email). */
  function jo_employee_handbook_path() {
      return __DIR__ . DIRECTORY_SEPARATOR . 'pdf' . DIRECTORY_SEPARATOR . 'Employee_Handbook.pdf';
  }


  /** Shared SMTP settings + long timeout (large PDF bundles). */
  function jo_phpmailer_apply_smtp_config(PHPMailer $mail) {
      $mail->isSMTP();
      $mail->Host       = 'hohcareers.com';
      $mail->SMTPAuth   = true;
      $mail->Username   = 'noreply@hohcareers.com';
      $mail->Password   = '9pw?TAk+z_ik';
  ...
  /**
   * HTML body for the employee handbook follow-up email.
   */
  function jo_build_handbook_email_body($recipientName = '') {
      $name = trim((string) $recipientName);
      $greeting = $name !== '' ? 'Dear ' . htmlspecialchars($name, ENT_QUOTES, 'UTF-8') . ',' : 'Dear Team Member,';
...
<body style="margin:0;padding:0;background-color:#f4f6f8;font-family:Arial,Helvetica,sans-serif;color:#333333;">
  <table role="presentation" width="100%" cellspacing="0" cellpadding="0" style="background-color:#f4f6f8;padding:24px 12px;">
    <tr>
      <td align="center">
        <table role="presentation" width="600" cellspacing="0" cellpadding="0" style="max-width:600px;width:100%;background-color:#ffffff;border:1px solid #e2e8f0;border-radius:6px;overflow:hidden;">
          <tr>
            <td style="background-color:#1e4d7b;padding:20px 28px;">
              <p style="margin:0;font-size:18px;font-weight:bold;color:#ffffff;letter-spacing:0.3px;">Employee Handbook</p>
              <p style="margin:6px 0 0;font-size:13px;color:#d9e6f2;">My Home Health Care Group</p>
            </td>
          </tr>
          <tr>
            <td style="padding:28px 28px 8px;font-size:15px;line-height:1.6;">
              <p style="margin:0 0 16px;">' . $greeting . '</p>
              <p style="margin:0 0 16px;">As part of your onboarding materials, please find your <strong>Employee Handbook</strong> attached to this message.</p>
              <p style="margin:0 0 16px;">We ask that you review the handbook carefully and keep a copy for your records. It contains important information about our policies, standards of conduct, and your rights and responsibilities as a team member.</p>
              <p style="margin:0 0 8px;"><strong>Attachment:</strong> Employee_Handbook.pdf</p>
            </td>
          </tr>
          <tr>
            <td style="padding:8px 28px 28px;font-size:15px;line-height:1.6;">
              <p style="margin:0 0 16px;">If you have any questions regarding the handbook or your onboarding documents, please contact your supervisor or the Human Resources department.</p>
              <p style="margin:0;">Sincerely,<br><strong>Human Resources</strong><br>My Home Health Care Group</p>
            </td>
          </tr>
          <tr>
            <td style="padding:16px 28px;background-color:#f8fafc;border-top:1px solid #e2e8f0;font-size:11px;line-height:1.5;color:#64748b;">
              <p style="margin:0;">This is an automated message sent from a notification-only address. Please do not reply to this email. If you need assistance, contact your supervisor or HR directly.</p>
            </td>
          </tr>
        </table>
      </td>
  ...
  /**
   * Plain-text fallback for the employee handbook email.
   */
  function jo_build_handbook_email_alt_body($recipientName = '') {
      $name = trim((string) $recipientName);
      $greeting = $name !== '' ? 'Dear ' . $name . ',' : 'Dear Team Member,';

      return $greeting . "\n\n"
        . "As part of your onboarding materials, please find your Employee Handbook attached to this message (Employee_Handbook.pdf).\n\n"
        . "Please review the handbook carefully and keep a copy for your records. It contains important information about our policies, standards of conduct, and your rights and responsibilities as a team member.\n\n"
        . "If you have any questions, please contact your supervisor or the Human Resources department.\n\n"
        . "Sincerely,\nHuman Resources\nMy Home Health Care Group\n\n"
        . "This is an automated message. Please do not reply to this email.";
  }

  /**
   * Second message with handbook only — avoids main message hitting size limits when many PDFs are attached.
   */
  function jo_send_handbook_email($recipientEmail, $handbookPath, $recipientName = '') {
      if ($recipientEmail === '' || ! is_readable($handbookPath)) {
          return false;
      }
      ...
      try {
          jo_phpmailer_apply_smtp_config($mail);
          $mail->setFrom('noreply@hohcareers.com', 'HOH Careers');
          $mail->addAddress($recipientEmail);
          $mail->isHTML(true);
          $mail->Subject = 'Your Employee Handbook – Please Review and Retain';
          $mail->Body = jo_build_handbook_email_body($recipientName);
          $mail->AltBody = jo_build_handbook_email_alt_body($recipientName);
          $mail->addAttachment($handbookPath, 'Employee_Handbook.pdf');
          $mail->send();
          return true;
      } catch (\Throwable $e) {
          error_log('JO handbook mail send failed: ' . $e->getMessage());
          return false;
      }
  }


  function send_mail($file_name,$email,$name,$date,$post=array(),$receiver=false,$pdf_file1="",$pdf_jo_file_name_2="",$pdf_jo_file_name_3="",$pdf_jo_file_name_4="",$pdf_file2="",$file_name_pdf="",$file_name_pdf_2="",$file_name_pdf_3="",$pdf_orientation_topics_file_name=""){
...
// echo $html;die();
   ...
   //    $content = chunk_split(base64_encode($content));
    ...
    /* Set the email header */
     ...
     $handbookPath = jo_employee_handbook_path();

     $includeHandbook = !$receiver && is_readable($handbookPath);
...
Admin
 <br><br>";
    ...
    $mail->setFrom('noreply@hohcareers.com', 'Careers');
        ...
        // Attach only if files exist
        $jo1 = !empty($pdf_file1) ? ('pdf_files/'.$pdf_file1) : '';
        if (!empty($jo1) && file_exists($jo1)) {
          $mail->addAttachment($jo1);
        }

        if (!empty($pdf_file2) && file_exists($pdf_file2)) {
          $mail->addAttachment($pdf_file2);
        }

        if (!empty($file_name_pdf) && file_exists($file_name_pdf)) {
          $mail->addAttachment($file_name_pdf);
        }

        if (!empty($pdf_jo_file_name_2)) {
          $p = 'pdf_files/'.$pdf_jo_file_name_2;
          if (file_exists($p)) {
        ...
        if (!empty($pdf_jo_file_name_3)) {
          $p = 'pdf_files/'.$pdf_jo_file_name_3;
          if (file_exists($p)) {
            $mail->addAttachment($p);
          }
        }
        ...
        if (!empty($pdf_jo_file_name_4)) {
          $p = 'pdf_files/'.$pdf_jo_file_name_4;
          if (file_exists($p)) {
            $mail->addAttachment($p);
          }
        }

        if (!empty($pdf_orientation_topics_file_name)) {
          $p = 'pdf_files/'.$pdf_orientation_topics_file_name;
          if (file_exists($p)) {
            $mail->addAttachment($p);
          }
        }

        if (!empty($file_name_pdf_2) && file_exists($file_name_pdf_2)) {
          $mail->addAttachment($file_name_pdf_2);
        }

        if (!empty($file_name_pdf_3) && file_exists($file_name_pdf_3)) {
        ...
        try {
          $mail->send();
          if ($includeHandbook) {
            jo_send_handbook_email($email, $handbookPath, $name);
          }
        } catch (\Throwable $e) {
          error_log('JO mail send failed: ' . $e->getMessage());
        }
  ...
  function save_pdf($html,$signature1,$signature2,$signature3,$name="",$pdf_jo_file_name_2="",$pdf_jo_file_name_3="",$pdf_jo_file_name_4="",$agr_name="",$jo_pdf_file="",$jo_pdf_file_2="",$jo_pdf_file_3="",$jo_pdf_file_4="",$ack_pdf_file="",$ack_2_pdf_file="",$ack_3_pdf_file="",$post=array(),$orientation_topics_file="",$pdf_orientation_topics_file_name=""){
// echo "adsa";die();
    // echo $name;die();
...
// echo $pdf_file;die();
    ...
    if (!empty($pdf_file) && file_exists($pdf_file)) {
      ...
      if (empty($get_dimensions) || empty($get_dimensions['num_pages'])) {
        $get_dimensions = ['num_pages' => 1, 'sig_x' => 60, 'sig_y' => 220, 'label_x' => 150, 'label_y' => 230];
      }
      for($x=1;$x<=$get_dimensions['num_pages'];$x++){
        ...
        if($x === $get_dimensions['num_pages']){
          // Move signature down a bit for better alignment
          $pdf->Image($placed_signature,$get_dimensions['sig_x'],$get_dimensions['sig_y'] + 3,60,20,"PNG");
          $pdf->SetFont('Arial','',9);  
          $pdf->SetXY($get_dimensions['label_x'],$get_dimensions['label_y']);  
          $pdf->Write(0, $date_label);
          // Add name near signature area for Job Description
      ...
      if(!empty($jo_pdf_file_2) && file_exists($jo_pdf_file_2)){
            $pdf = new FPDI();  
        ...
        for($x=1;$x<=$get_dimensions['num_pages'];$x++){
             ...
             $tplIdx = $pdf->importPage($x);  
              ...
              if($x === $get_dimensions['num_pages']){
                // Last step (Orientation Acknowledgment): nudge signature slightly right and down
                $pdf->Image($placed_signature,$get_dimensions['sig_x'] + 16,$get_dimensions['sig_y'] + 4,60,20,"PNG");
      ...
      if(!empty($jo_pdf_file_3) && file_exists($jo_pdf_file_3)){
            $pdf = new FPDI();  
        ...
        if(!empty($jo_pdf_file_4) && file_exists($jo_pdf_file_4)){
            $pdf = new FPDI();  
      ...
      // Orientation Topics Covered PDF (no extra pad; reuse existing signature + date)
      if(!empty($orientation_topics_file) && file_exists($orientation_topics_file) && !empty($pdf_orientation_topics_file_name)){
            $pdf = new FPDI();
            // Use actual page count from source file to ensure all pages are included
            $page_count = $pdf->setSourceFile($orientation_topics_file);
            $get_dimensions = get_x_y_dimensions($orientation_topics_file);
            if (empty($get_dimensions)) {
              $get_dimensions = [];
            }
            if (empty($get_dimensions['num_pages'])) {
              $get_dimensions['num_pages'] = !empty($page_count) ? (int)$page_count : 1;
            }
            if (empty($get_dimensions['sig_x']))   { $get_dimensions['sig_x'] = 60; }
            if (empty($get_dimensions['sig_y']))   { $get_dimensions['sig_y'] = 220; }
            if (empty($get_dimensions['label_x'])) { $get_dimensions['label_x'] = 150; }
            if (empty($get_dimensions['label_y'])) { $get_dimensions['label_y'] = 230; }

            // Dedicated placement for Orientation Topics Covered template
            // so employee name/sign/date land on the visible lines.
            $name_x = 70;
            $name_y = 97;
            $sig_w  = 60;
            $sig_h  = 36;
            // Liana Berzins agency signature (smaller than employee signature)
            $agency_sig_w = 34;
            $agency_sig_h = 17;
            // Agency representative line (below employee signature line)
            $agency_sig_x = 100;
            $agency_sig_y = 114;
            $agency_date_x = 158;
            $agency_date_y = 127;
            $agency_name_x = 85;
            $agency_name_y = 127;
            if (basename($orientation_topics_file) === 'Orientation Topics Covered.pdf') {
              $get_dimensions['sig_x'] = 73;
              $get_dimensions['sig_y'] = 94;
              $get_dimensions['label_x'] = 158;
              $get_dimensions['label_y'] = 112;
            }
            ...
            for($x=1;$x<=$get_dimensions['num_pages'];$x++){
              ...
              if($x === $get_dimensions['num_pages']){
                $pdf->Image($placed_signature,$get_dimensions['sig_x'],$get_dimensions['sig_y'],$sig_w,$sig_h,"PNG");
                $pdf->SetFont('Arial','',9);
                $pdf->SetXY($get_dimensions['label_x'],$get_dimensions['label_y']);
                $pdf->Write(0, $date_label);
                ...
                // Add agency representative signature/date on lower line
                $agency_signature_file = 'staff_signatures/liana_berzins_signature.png';
                if (file_exists($agency_signature_file)) {
                  $pdf->Image($agency_signature_file,$agency_sig_x,$agency_sig_y,$agency_sig_w,$agency_sig_h,"PNG");
                } else {
                  // Fallback to employee signature if staff signature file is missing
                  $pdf->Image($placed_signature,$agency_sig_x,$agency_sig_y,$agency_sig_w,$agency_sig_h,"PNG");
                }
                $pdf->SetXY($agency_name_x,$agency_name_y);
                $pdf->Write(0, 'Liana Berzins');
                $pdf->SetXY($agency_date_x,$agency_date_y);
                $pdf->Write(0, $date_label);
              }
            }

            $pdf_file_name = "pdf_files/".$pdf_orientation_topics_file_name;
...
// echo $agr_name;die();
    ...
    if($post['organization'] == 'Both'){
      ...
      $pdf->Image($placed_signature,50,210,60,20,"PNG");
       ...
       // Move signature and date up to align with line
       $pdf->Image($placed_signature,50,203,60,20,"PNG");
