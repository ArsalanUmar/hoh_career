...
use PHPMailer\PHPMailer\SMTP;
  ...
  use \setasign\Fpdi\Fpdi;
    ...
    if(!empty($_POST)){
      ...
      $hh_file_path = $record_details['hh_file_path'];
...
// echo "<pre>",print_r($post),"</pre>";die();
    $pdf_file_name = "";
     ...
     // die();
        extract($_POST);
        $file_name = "HH_SIGNED_".$first_name." ".$last_name."_1_".$date.".jpg";
   
        $file_name_pdf =  str_replace("pdf_files/", "pdf_files/HH_SIGNED_", $pdf_file_path);
       ...
       // $pdf_agr_file_name = "pdf_files/AGR_".$file_name_pdf.' '.date('Y-m-d').'.pdf';
        ...
        if($response){
         ...
         save_pdf($html,"signatures/".$file_name,$file_name_pdf,$hh_file_path,$post);
          ...
          // if($send_mail){
            header("Location: stf_pao_form.php?ref=".base64_encode($ref)."&s=1");exit;
...
// echo $html;die();
   ...
   //   $file_name = "<attachment file name>";
    ...
    /* Set the email header */
      ...
      $message = "$m".$eol;
     $message .= "<br>See attachment to read the copy of your personnel orientation acknowledgement, and please keep in your records. <br><br>
Thank you so much!";
  ...
  function save_pdf($html,$signature1,$file_name_pdf="",$hh_file_path="",$post=array()){

    $pdf_file = str_replace('_SIGNED_', "_", $hh_file_path);//'pdf/ho_template.pdf';//$jo_pdf_file;// "pdf/.pdf";
    $date=date('Y-m-d');
    ...
    $pdf->SetXY(55 ,47);  
     ...
     $pdf->Image($signature1,78,220,60,20,"PNG");
     $pdf->SetXY(157 ,232);  
     $pdf->Write(0, $date_label);
     $pdf->Output($file_name_pdf,'F');//exit;

     $pdf_file_2 = str_replace("HH_", "HH_2_", $hh_file_path);
      $pdf_file_2 = str_replace("HH_2_SIGNED_", "HH_2_", $pdf_file_2);
     // echo $pdf_file_2;die();
     if(file_exists($pdf_file_2)){
      $file_name_pdf_2 = str_replace("HH_SIGNED_", "HH_2_SIGNED_", $file_name_pdf);

       $pdf = new FPDI();  
        // add a page
        $pdf->AddPage();  
        // set the sourcefile  
        $pdf->setSourceFile($pdf_file_2);  
        // import page 1  
        $tplIdx = $pdf->importPage(1);  
        $pdf->useTemplate($tplIdx, 10, 10, 200);  
        ...
        $pdf->SetXY(55 ,47);  
         ...
         $pdf->SetXY(157 ,232);  
         $pdf->Write(0, $date_label);
         $pdf->Output($file_name_pdf_2,'F');//exit;
     }

     $pdf_file_3 = str_replace("HH_", "HH_3_", $hh_file_path);
     $pdf_file_3 = str_replace("HH_3_SIGNED_", "HH_3_", $pdf_file_3);
     if(file_exists($pdf_file_3)){
      $file_name_pdf_3 = str_replace("HH_SIGNED_", "HH_3_SIGNED_", $file_name_pdf);

       $pdf = new FPDI();  
        // add a page
        $pdf->AddPage();  
        // set the sourcefile  
        $pdf->setSourceFile($pdf_file_3);  
        // import page 1  
        $tplIdx = $pdf->importPage(1);  
        $pdf->useTemplate($tplIdx, 10, 10, 200);  
        ...
        $pdf->SetXY(55 ,47);  
         // $pdf->Write(0, ucwords($post['name']));
         $pdf->Image($signature1,78,210,60,20,"PNG");
         $pdf->SetXY(157 ,226);  
         $pdf->Write(0, $date_label);
         $pdf->Output($file_name_pdf_3,'F');//exit;
     }
