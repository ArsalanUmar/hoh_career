     ...
     foreach($data as $k=>$v){
         ...
         // $content .= "<td>".ucwords($v->create_date)."</td>";
          ...
          if($tbl_name=='tbl_job_applications' && !empty($v->zip_code)){
            ...
            $content .= "<td><a href='".base_url()."employee_certificates/".$base64_id_encode."'><button id=''  class='pads btn btn-block btn-primary btn-sm'><span><i class='fa fa-eye'></i></span> View Tests</button></a></td>";
          }else{
             $content .= "<td><button id='edit' class='pads btn btn-block btn-success btn-sm edit'><span id='edit'><i class='fa fa-edit'></i> Edit</span></button><button id='remove'  class='pads btn btn-block btn-danger btn-sm remove'><span><i class='fa fa-times'></i> Delete</span></button><a href='".base_url().'views/'.$base_encode."'></td>";
          }
