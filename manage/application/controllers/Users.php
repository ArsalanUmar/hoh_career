...
class Users extends CI_Controller {
...

	{
		...
		if($response){
			...
			// After successful login, send user to welcome dashboard
		...
		// echo "<pre>",print_r($this->input->post()),"</pre>";die();
		 ...
		 if($tbl=='tbl_job_applications'){
		   ...
		   if($tbl=='tbl_test_certificates'){
 				...
 				if($tbl=='tbl_custom_certificates'){
					// $send = array("status"=>'archived'); 
					// $this->umodel->update($send,$id,'staff');
					$this->db->delete($tbl , array('id' => $id)); 
		         ...
		         if($tbl=='tbl_influenza_forms'){
					// $send = array("status"=>'archived'); 
					// $this->umodel->update($send,$id,'staff');
					$this->db->delete($tbl , array('id' => $id)); 
		         }
		         if($tbl=='tbl_inservice_logs'){
					$this->db->delete($tbl , array('id' => $id)); 
		         }
	...
	public function side_nav(){
		...
		$nav = array();
			...
			$nav = array(
					 ...
					 'Manage Job Applications' => array("icon"=> "fa fa-briefcase" , "url" => "signatures"),
						 'Competency Form' => array("icon"=> "fa fa-certificate" , "url" => "../competency.php"),
					 'Performance Evaluation' => array(
					 	' Registered Nurse' => array("icon"=> "fa fa-user-md" , "url" => "../registered_nurse.php"),
					 	' LPN/LVN' => array("icon"=> "fa fa-stethoscope" , "url" => "../L_practical.php"),
					 	' Home Health Aide' => array("icon"=> "fa fa-heartbeat" , "url" => "../chh_certificate.php"),
					 	' Office Staff' => array("icon"=> "fa fa-users" , "url" => "../office_staff.php"),
					 	' View Saved Evaluations' => array("icon"=> "fa fa-folder-open" , "url" => "../view_saved_rn_evaluations.php")
					 ),
						 
						 'Manage Influenza' => array(
						 ...
						 // 'Manage Custom Certificates' => array("icon"=> "fa fa-star" , "url" => "custom-certificates"),

						 // 'Manage Tests' => array("icon"=> "fa fa-book" , "url" => "certificates"),
						  // 'Compentency Form' => array("icon"=> "fa fa-certificate" , "url" => "../competency.php"),
						  // 'Create Certificate' => array("icon"=> "fa fa-certificate" , "url" => "../certificates.php"), 
						  // 'Custom Certificate' => array("icon"=> "fa fa-certificate" , "url" => "../custom_certificates.php"),
...
// echo "<pre>",print_r($response),"</pre>";die();
	...
	public function welcome(){
		...
		// Simple dashboard-style welcome content using existing layout
		$user_safe = htmlspecialchars($data['user'], ENT_QUOTES, 'UTF-8');
		$base_url = base_url();
		$data['content'] = "
	...
	public function signatures(){
		...
		// var_dump('loop');die();
 		...
 		$filter = false;
 			...
 			$response = $this->umodel->get_applications();
...
// echo "<pre>",print_r($response),"</pre>";die();
 		...
 		$data['content'] = createTable($fields,$table_name,$response,$data['current_page'],$filter,false);
	...
	public function manual_signatures(){
			// echo "<pre>",print_r($_SESSION),"</pre>";die();
		if(!$this->check_token()){
		...
		// var_dump('loop');die();
 		...
 		$filter = false;
 			...
 			$response = $this->umodel->get_manual_applications();
...
// echo "<pre>",print_r($response),"</pre>";die();
 		...
 		$data['content'] = createTable($fields,$table_name,$response,$data['current_page'],$filter,true);
	...
	public function archived_signatures(){
			// echo "<pre>",print_r($_SESSION),"</pre>";die();
		if(!$this->check_token()){
		...
		$data['logged_in'] = true;
 		...
 		$data['page'] = 'Archived Applications';
		...
		// var_dump('loop');die();
 		...
 		$filter = false;
 			...
 			$response = $this->umodel->get_archived_applications();
...
// echo "<pre>",print_r($response),"</pre>";die();
 		...
 		$data['content'] = createTable($fields,$table_name,$response,$data['current_page'],$filter,false,true);
...
// echo "<pre>",print_r($response),"</pre>";die();
   ...
   public function certificates(){
		...
		// var_dump('loop');die();
 	 	...
 	 	$fields = array(	'employee_id'=>array('type'=>'text','placeholder'=>'ID','label' => 'Control ID'),
...
// echo "<pre>",print_r($response),"</pre>";die();
	 ...
	 public function manage_custom_certificates(){
			// echo "<pre>",print_r($_SESSION),"</pre>";die();
		if(!$this->check_token()){
		...
		$data['logged_in'] = true;
 		...
 		$data['page'] = 'Manage Custom Certificates';
		...
		// var_dump('loop');die();
 	 	...
 	 	$fields = array(	'id'=>array('type'=>'text','placeholder'=>'ID','label' => 'Control ID'),
 	 		'cert_name'=>array('type'=>'text','placeholder'=>'Certificate Name','label' => 'Certificate Name'),

 						'employee_name'=>array('type'=>'text','placeholder'=>'Employee Name','label' => 'Employee Name'),
 						// 'signature_path'=>array('type'=>'file','placeholder'=>'Signature','label' => 'Signature File'),
 					

 						'cert_file_path'=>array('type'=>'pdf_path','placeholder'=>'Certificate File','label' => 'Certificate File'),
 						// 'date_created'=>array('type'=>'text','placeholder'=>'Date Added','label' => ''),
 		...
 		$table_name= 'tbl_custom_certificates';
 		...
 		$response = $this->umodel->get_custom_certificates();
	 ...
	 public function manage_service_logs(){
			// echo "<pre>",print_r($_SESSION),"</pre>";die();
		if(!$this->check_token()){
		...
		$data['logged_in'] = true;
 		...
 		$data['page'] = 'In-Service Schedule';

 		$data['current_page'] = 'service_logs';
		...
		// var_dump('loop');die();
	 	...
	 	$fields = array(
						'id'=>array('type'=>'text','placeholder'=>'ID','label' => 'Control ID'),
						'employee_name'=>array('type'=>'text','placeholder'=>'Employee Name','label' => 'Employee Name'),
						'topic_duration'=>array('type'=>'text','placeholder'=>'Month, Topic and Duration','label' => 'Month, Topic and Duration'),
						'date_added'=>array('type'=>'text','placeholder'=>'Date Added','label' => 'Date Added'),
					);

 		$table_name= 'tbl_inservice_logs';
 		...
 		$response = $this->umodel->get_service_logs();
	 ...
	 public function influenza_forms(){
		...
		$data['logged_in'] = true;
 		...
 		$data['page'] = 'Manage Influenza Forms';
...
// echo "<pre>",print_r($response),"</pre>";die();
	...
	public function employee_certificates($ref=NULL){
		...
		$reference = base64_decode($ref);
		$response =  $this->umodel->get_certificates($reference);
		$custom_certificates =  $this->umodel->get_custom_certificates($reference);//$this->umodel->get_where('sign_forms',array('id'=>$ref));
		// echo $response;die();
		$data['logged_in'] = true;
 		...
 		$data['user'] = ucwords($this->session->userdata('name'));
			// echo "<pre>",print_r($custom_certificates),"</pre>";die();
		if(!empty($response)){
			$doc_info = $response[0];
			 		$data['content'] = view_certificates($response[0],$custom_certificates,'employee_certificates/'.$ref);
...
// die();
	...
	{	
		...
		$tbl_name = 'tbl_test_certificates';
         if($module=='waive_test'){
			$send = array("waived_test_file_path"=>NULL); 
			$this->umodel->update($send,$id,'test_certificates');
			// $this->db->delete($tbl , array('id' => $id)); 
         }

         if($module=='waive_cert'){
			$send = array("waived_certificate_file_path"=>NULL); 
			$this->umodel->update($send,$id,'test_certificates');
			// $this->db->delete($tbl , array('id' => $id)); 
         }

         if($module=='hepab_test'){
			$send = array("hepa_test_file_path"=>NULL); 
			$this->umodel->update($send,$id,'test_certificates');
			// $this->db->delete($tbl , array('id' => $id)); 
         }

         
         if($module=='hepab_cert'){
			$send = array("hepa_certificate_file_path"=>NULL); 
			$this->umodel->update($send,$id,'test_certificates');
         }
 
 		 if($module=='harass_test'){
			$send = array("harass_test_file_path"=>NULL); 
			$this->umodel->update($send,$id,'test_certificates');
			// $this->db->delete($tbl , array('id' => $id)); 
         }

         
         if($module=='harass_cert'){
			$send = array("harass_certificate_file_path"=>NULL); 
			$this->umodel->update($send,$id,'test_certificates');
         }

          if($module=='hygiene_test'){
			$send = array("hygiene_test_file_path"=>NULL); 
			$this->umodel->update($send,$id,'test_certificates');
			// $this->db->delete($tbl , array('id' => $id)); 
         }

         
         if($module=='hygiene_cert'){
			$send = array("hygiene_cert_file_path"=>NULL); 
			$this->umodel->update($send,$id,'test_certificates');
         }

         if($module=='emerg_test'){
			$send = array("emergency_test_file_path"=>NULL); 
			$this->umodel->update($send,$id,'test_certificates');
			// $this->db->delete($tbl , array('id' => $id)); 
         }

         
         if($module=='emerg_cert'){
			$send = array("emergency_certificate_file_path"=>NULL); 
			$this->umodel->update($send,$id,'test_certificates');
         }


         if($module=='covid_test'){
			$send = array("covid_test_file_path"=>NULL); 
			$this->umodel->update($send,$id,'test_certificates');
			// $this->db->delete($tbl , array('id' => $id)); 
         }

         
         if($module=='covid_cert'){
			$send = array("covid_certificate_file_path"=>NULL); 
			$this->umodel->update($send,$id,'test_certificates');
         }


         if($module=='gluco_test'){
			$send = array("glucometer_test_file_path"=>NULL); 
			$this->umodel->update($send,$id,'test_certificates');
			// $this->db->delete($tbl , array('id' => $id)); 
         }

         
         if($module=='gluco_cert'){
			$send = array("glucometer_certificate_file_path"=>NULL); 
			$this->umodel->update($send,$id,'test_certificates');
         }

          if($module=='bloodpatho_test'){
			$send = array("bloodpatho_test_file_path"=>NULL); 
			$this->umodel->update($send,$id,'test_certificates');
			// $this->db->delete($tbl , array('id' => $id)); 
         }

         
         if($module=='bloodpatho_cert'){
			$send = array("bloodpatho_certificate_file_path"=>NULL); 
			$this->umodel->update($send,$id,'test_certificates');
         }
	...
	public function delete_custom_certificate()
	...
	{	
		...
		$id = $this->input->post('ref');	
		$module = $this->input->post('module');	
		$tbl_name = 'tbl_custom_certificates';
