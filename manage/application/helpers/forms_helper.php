<?php
  ...
  function createTable($fields = array(), $tbl_name='' , $data = array(), $cur_page='', $filter = false ,$manual_application = false,$archived_application=false){
      // var_dump($manual_application);
    ...
    $content .= "<div class='success' style='display:$display'>$message</div>";
     ...
     if(isset($_GET['q'])){
          ...
          $content .= "<div id = 'modal'><button id='create' class='btn btn-block btn-primary fifty'><i class='fa fa-plus'></i>&nbsp;&nbsp;Add</button></div>";
            ...
            if(!$archived_application){

             $content .="<div><a href='".base_url()."archived_signatures'><button id='archived' class='btn btn-block btn-warning fifty'  style='max-width:200px;  '><i class='fa fa-eye'></i>&nbsp;&nbsp;View Archived Employee</button></a></div>";
                
              if($manual_application === false){

                $content .= "<div id = 'modal'><a href='".base_url()."manual_signatures'><button class='btn btn-block btn-info fifty'  style='max-width:200px;    float: right;  margin-top: -35px;'><i class='fa fa-eye'></i>&nbsp;&nbsp;View Manual Employee</button></a></div>";
              }else{

                $content .= "<div id = 'modal'><a href='".base_url()."signatures'><button class='btn btn-block btn-info fifty'  style='max-width:200px;    float: right;  margin-top: -35px;'><i class='fa fa-eye'></i>&nbsp;&nbsp;View Job Applications</button></a></div>";
              
              }
              $content .= "<div id = 'modal'><button id='create' class='btn btn-block btn-primary fifty'  style='max-width:200px;margin-top:6px;'><i class='fa fa-plus'></i>&nbsp;&nbsp;Add Employee Signature</button></div>";
            }else{
               $content .= "<div id = 'modal'><a href='".base_url()."signatures'><button class='btn btn-block btn-info fifty'  style='max-width:200px;    '><i class='fa fa-eye'></i>&nbsp;&nbsp;View Job Applications</button></a></div>";
    ...
    $content .= "<div class='table-responsive mt-2'><table id='".$tbl_name."' name='". $tbl_name."' class='table table-bordered table-striped'>
...
// echo $tbl_name;die();
    ...
    // echo "<pre>",print_r($data),"</pre>";die();
     ...
     foreach($data as $k=>$v){
         if(isset($v->staff_id) && $tbl_name == "tbl_staff"){
          $v->id = $v->staff_id;
         }
         $content .= "<tr ref= ".$v->id .">";
         $base_encode = base64_encode($v->id);
         // if(in_array($v->id, $in_id) || $v->id =="0"){
         //  continue;
         // }
         // $in_id[] = $v->id;
          // if($tbl_name=='admin' ){
...
// echo "<pre>",print_r($v),"</pre>";//die();
             ...
             if($kf == 'password'){
                ...
                if($v->$kf == '0'){
                  ...
                  $content .= "<td name='$kf'><label class='label label-success'>Consent</label></td>";
              	 ...
              	 $edit_date_btn = NULL;
                 if($tbl_name=='tbl_influenza_forms'){
                    $edit_date_btn = "<a href='".base_url().'../influenza_update_form.php?r='.$v->id."' target='_blank'><button id=''  class='pads btn btn-block btn-primary btn-sm'><span><i class='fa fa-edit'></i>&nbsp;Edit Date</span></button></a><button id='remove'  class='pads btn btn-block btn-danger btn-sm remove'><span><i class='fa fa-times'></i> Delete</span></button>";
                 }
                if(!empty($v->$kf)){

                   $content .= "<td name='$kf'><a href='".base_url().'../'.$v->$kf."' target='_blank'><button id='view'  class='pads btn btn-block btn-success btn-sm'><span><i class='fa fa-file-o'></i>&nbsp;View File</span></button></a>".$edit_date_btn."</td>";

                }else{
                  if($tbl_name=='tbl_influenza_forms'){
                    $content .= "<td name='$kf'><a href='".base_url().'../influenza_form_admin.php?ref='.$v->id."'><button type='button' class='pads btn btn-block btn-warning btn-sm ' name='influenza_process_btn'><i class='fa fa-refresh'></i>&nbsp;Process PDF</button></a></td>";
                  }else{
              ...
              }elseif($tbl_name == 'tbl_inservice_logs' && $kf == 'topic_duration'){

                 $val = isset($v->$kf) ? $v->$kf : '';
                 $content .= "<td name='$kf' class='service-log-topics-duration' style='white-space:normal; max-width:380px; vertical-align:top; padding:6px;'>".$val."</td>";
         ...
         if($tbl_name != 'tbl_influenza_forms' ){
          if($tbl_name=='tbl_job_applications' ){
             if(!$archived_application){
                $content .= "<td><a href='".base_url()."view/".$base64_id_encode."'><button id=''  class='pads btn btn-block btn-primary btn-sm'><span><i class='fa fa-eye'></i></span> View Details</button></a><button id='remove'  class='pads btn btn-block btn-danger btn-sm remove'><span><i class='fa fa-times'></i> Delete</span></button><button id='archived'  class='pads btn btn-block btn-warning btn-sm archived'><span><i class='fa fa-trash'></i>&nbsp;Archived</span></button></td>";
              }else{
                  $content .= "<td><button id='change_to_active'  class='pads btn btn-block btn-primary btn-sm change_to_active'><span><i class='fa fa-refresh'></i>&nbsp;Reactivate</span></button></td>";
              }
          }elseif($tbl_name=='tbl_test_certificates' ){
                  $base64_id_encode = base64_encode($v->employee_id);
          ...
          }elseif($tbl_name=='tbl_custom_certificates' ){
            $content .= "<td><a href='".base_url()."../".$v->cert_file_path."' download><button id=''  class='pads btn btn-block btn-primary btn-sm'><span><i class='fa fa-arrow-down'></i></span> Download</button></a><button id='remove'  class='pads btn btn-block btn-danger btn-sm remove'><span><i class='fa fa-times'></i> Delete</span></button></td>";
         ...
         }elseif($tbl_name=="tbl_inservice_logs"){
                   $bulk_btn = '';
                   if (!empty($v->bulk_certificate_url) && strpos($v->bulk_certificate_url, 'topics=') !== false) {
                     $bulk_btn = "<a href='".base_url().$v->bulk_certificate_url."' target='_blank'><button type='button' class='pads btn btn-block btn-info btn-sm' style='max-width: 120px;'><span><i class='fa fa-print'></i> Bulk Print</span></button></a>&nbsp;";
                   }
                   $content .= "<td>".$bulk_btn."<a href='".base_url().'../'.$v->pdf_path."' target='_blank' download><button id='gen_consent' class='pads btn btn-block btn-primary btn-sm ' style='    max-width: 120px;'><span ><i class='fa fa-download'></i> Download File</span></button></a>&nbsp;<a href='".base_url()."../create_service_log.php?ref=".$base64_id_encode."' target='_blank'><button id='edit' class='pads btn btn-block btn-success btn-sm '><span id=''><i class='fa fa-edit'></i> Edit</span></button></a>&nbsp;<button id='remove'  class='pads btn btn-block btn-danger btn-sm remove'><span><i class='fa fa-times'></i> Delete</span></button><a href='".base_url().'views/'.$base_encode."'></td>";
          }else{
            if (empty($v->legal_consent_path)){
...
<script src="'.base_url().'../js/sketch.js"></script>  <script src="'.base_url().'../lang/jquery.signfield-en.min.js"></script>
 ...
 function view_certificates($response,$custom_certificates=array(),$cur_page=""){
  ...
  // echo "<pre>",print_r($response),"</pre>";die();
   ...
   if($s=='4'){
    ...
    $content .= "<div class='success' style='display:block'>File successfully deleted!</div>";
   }
        ...
        $test_file_array = array();
         ...
         $content .= "<div class='form-group'>";
          ...
          if(!empty($response->waived_test_file_path)){
            $content .="<div class='col-md-6'>";
              $content .= "<p>&bull;&nbsp;<a href='".base_url()."../".$response->waived_test_file_path."' target='_blank'>WAIVE TEST FILE</a>&nbsp;&nbsp;<a class='btn btn-xs btn-primary' target='_blank' href='".base_url()."../".$response->waived_test_file_path."' download>Download</a>&nbsp;&nbsp;<button class='btn btn-xs btn-danger btn-test-delete'  type='button' module='waive_test' ref='".$response->id."'>Delete</button></p>";
             $content .="</div>";
           }
           if(!empty($response->waived_certificate_file_path)){
              $content .="<div class='col-md-6'>";
              $content .= "<p>&bull;&nbsp;<a href='".base_url()."../".$response->waived_certificate_file_path."' target='_blank'>WAIVE CERTIFICATE</a>&nbsp;&nbsp;<a class='btn btn-xs btn-primary' target='_blank' href='".base_url()."../".$response->waived_certificate_file_path."' download>Download</a>&nbsp;&nbsp;<button class='btn btn-xs btn-danger btn-test-delete'  type='button' module='waive_cert' ref='".$response->id."'>Delete</button></p>";
             $content .="</div>";
            }
              $content .="</div>";
            ...
            $content .="<div class='col-md-6'>";
              $content .= "<p>&bull;&nbsp;<a href='".base_url()."../".$response->hepa_test_file_path."' target='_blank'>HEPATITIS B TEST FILE</a>&nbsp;&nbsp;<a class='btn btn-xs btn-primary' target='_blank' href='".base_url()."../".$response->hepa_test_file_path."' download>Download</a>&nbsp;&nbsp;<button class='btn btn-xs btn-danger btn-test-delete'  type='button' module='hepab_test' ref='".$response->id."'>Delete</button></p>";
             $content .="</div>";
             }
            if(!empty($response->hepa_certificate_file_path)){
              $content .="<div class='col-md-6'>";
              $content .= "<p>&bull;&nbsp;<a href='".base_url()."../".$response->hepa_certificate_file_path."' target='_blank'>HEPATITIS B CERTIFICATE</a>&nbsp;&nbsp;<a class='btn btn-xs btn-primary' target='_blank' href='".base_url()."../".$response->hepa_certificate_file_path."' download>Download</a>&nbsp;&nbsp;<button class='btn btn-xs btn-danger btn-test-delete'  type='button' module='hepab_cert' ref='".$response->id."'>Delete</button></p>";
             $content .="</div>";
            }
              $content .="</div>";
            ...
            $content .="<div class='col-md-6'>";
              $content .= "<p>&bull;&nbsp;<a href='".base_url()."../".$response->harass_test_file_path."' target='_blank'>HARASSMENT TEST FILE</a>&nbsp;&nbsp;<a class='btn btn-xs btn-primary' target='_blank' href='".base_url()."../".$response->harass_test_file_path."' download>Download</a>&nbsp;&nbsp;<button class='btn btn-xs btn-danger btn-test-delete'  type='button' module='harass_test' ref='".$response->id."'>Delete</button></p>";
             $content .="</div>";
           }
             if(!empty($response->harass_certificate_file_path)){
              $content .="<div class='col-md-6'>";
              $content .= "<p>&bull;&nbsp;<a href='".base_url()."../".$response->harass_certificate_file_path."' target='_blank'>HARASSMENT CERTIFICATE</a>&nbsp;&nbsp;<a class='btn btn-xs btn-primary' target='_blank' href='".base_url()."../".$response->harass_certificate_file_path."' download>Download</a>&nbsp;&nbsp;<button class='btn btn-xs btn-danger btn-test-delete'  type='button' module='harass_cert' ref='".$response->id."'>Delete</button></p>";
             $content .="</div>";
              }
            ...
            if(!empty($response->hygiene_test_file_path)){
              $content .="<div class='col-md-6'>";
                $content .= "<p>&bull;&nbsp;<a href='".base_url()."../".$response->hygiene_test_file_path."' target='_blank'>HAND HYGIENE TEST FILE</a>&nbsp;&nbsp;<a class='btn btn-xs btn-primary' target='_blank' href='".base_url()."../".$response->hygiene_test_file_path."' download>Download</a>&nbsp;&nbsp;<button class='btn btn-xs btn-danger btn-test-delete'  type='button' module='hygiene_test' ref='".$response->id."'>Delete</button></p>";
               $content .="</div>";
              }
              if(!empty($response->hygiene_certificate_file_path)){
                $content .="<div class='col-md-6'>";
                $content .= "<p>&bull;&nbsp;<a href='".base_url()."../".$response->hygiene_certificate_file_path."' target='_blank'>HAND HYGIENE CERTIFICATE</a>&nbsp;&nbsp;<a class='btn btn-xs btn-primary' target='_blank' href='".base_url()."../".$response->hygiene_certificate_file_path."' download>Download</a>&nbsp;&nbsp;<button class='btn btn-xs btn-danger btn-test-delete'  type='button' module='hygiene_cert' ref='".$response->id."'>Delete</button></p>";
                $content .="</div>";
              }
              $content .="</div>";
           ...
           if(!empty($response->emergency_test_file_path)){
            $content .="<div class='col-md-6'>";
              $content .= "<p>&bull;&nbsp;<a href='".base_url()."../".$response->emergency_test_file_path."' target='_blank'> EMERGENCY TEST FILE</a>&nbsp;&nbsp;<a class='btn btn-xs btn-primary' target='_blank' href='".base_url()."../".$response->emergency_test_file_path."' download>Download</a>&nbsp;&nbsp;<button class='btn btn-xs btn-danger btn-test-delete'  type='button' module='emerg_test' ref='".$response->id."'>Delete</button></p>";
             $content .="</div>";
           }
            if(!empty($response->emergency_certificate_file_path)){
                $content .="<div class='col-md-6'>";
                $content .= "<p>&bull;&nbsp;<a href='".base_url()."../".$response->emergency_certificate_file_path."' target='_blank'> EMERGENCY  CERTIFICATE</a>&nbsp;&nbsp;<a class='btn btn-xs btn-primary' target='_blank' href='".base_url()."../".$response->emergency_certificate_file_path."' download>Download</a>&nbsp;&nbsp;<button class='btn btn-xs btn-danger btn-test-delete'  type='button' module='emerg_cert' ref='".$response->id."'>Delete</button></p>";
               $content .="</div>";
           ...
           if(!empty($response->covid_test_file_path)){
            $content .="<div class='col-md-6'>";
              $content .= "<p>&bull;&nbsp;<a href='".base_url()."../".$response->covid_test_file_path."' target='_blank'> COVID TEST FILE</a>&nbsp;&nbsp;<a class='btn btn-xs btn-primary' target='_blank' href='".base_url()."../".$response->covid_test_file_path."' download>Download</a>&nbsp;&nbsp;<button class='btn btn-xs btn-danger btn-test-delete'  type='button' module='covid_test' ref='".$response->id."'>Delete</button></p>";
             $content .="</div>";
            }

           if(!empty($response->covid_certificate_file_path)){
              $content .="<div class='col-md-6'>";
              $content .= "<p>&bull;&nbsp;<a href='".base_url()."../".$response->covid_certificate_file_path."' target='_blank'> COVID  CERTIFICATE</a>&nbsp;&nbsp;<a class='btn btn-xs btn-primary' target='_blank' href='".base_url()."../".$response->covid_certificate_file_path."' download>Download</a>&nbsp;&nbsp;<button class='btn btn-xs btn-danger btn-test-delete'  type='button' module='covid_cert' ref='".$response->id."'>Delete</button></p>";
             $content .="</div>";
            }
              $content .="</div>";
           ...
           if(!empty($response->glucometer_test_file_path)){
            $content .="<div class='col-md-6'>";
              $content .= "<p>&bull;&nbsp;<a href='".base_url()."../".$response->glucometer_test_file_path."' target='_blank'> GLUCOMETER TEST FILE</a>&nbsp;&nbsp;<a class='btn btn-xs btn-primary' target='_blank' href='".base_url()."../".$response->glucometer_test_file_path."' download>Download</a>&nbsp;&nbsp;<button class='btn btn-xs btn-danger btn-test-delete'  type='button' module='gluco_test' ref='".$response->id."'>Delete</button></p>";
             $content .="</div>";
            }
             if(!empty($response->glucometer_certificate_file_path)){
              $content .="<div class='col-md-6'>";
              $content .= "<p>&bull;&nbsp;<a href='".base_url()."../".$response->glucometer_certificate_file_path."' target='_blank'> GLUCOMETER  CERTIFICATE</a>&nbsp;&nbsp;<a class='btn btn-xs btn-primary' target='_blank' href='".base_url()."../".$response->glucometer_certificate_file_path."' download>Download</a>&nbsp;&nbsp;<button class='btn btn-xs btn-danger btn-test-delete'  type='button' module='gluco_cert' ref='".$response->id."'>Delete</button></p>";
             $content .="</div>";
             }
              $content .="</div>";
            ...
            if(!empty($response->bloodpatho_test_file_path)){
              $content .="<div class='col-md-6'>";
                $content .= "<p>&bull;&nbsp;<a href='".base_url()."../".$response->bloodpatho_test_file_path."' target='_blank'> BLOOD PATHOGEN TEST FILE</a>&nbsp;&nbsp;<a class='btn btn-xs btn-primary' target='_blank' href='".base_url()."../".$response->bloodpatho_test_file_path."' download>Download</a>&nbsp;&nbsp;<button class='btn btn-xs btn-danger btn-test-delete'  type='button' module='bloodpatho_test' ref='".$response->id."'>Delete</button></p>";
               $content .="</div>";
           }

           if(!empty($response->bloodpatho_certificate_file_path)){
              $content .="<div class='col-md-6'>";
              $content .= "<p>&bull;&nbsp;<a href='".base_url()."../".$response->bloodpatho_certificate_file_path."' target='_blank'> BLOOD PATHOGEN  CERTIFICATE</a>&nbsp;&nbsp;<a class='btn btn-xs btn-primary' target='_blank' href='".base_url()."../".$response->bloodpatho_certificate_file_path."' download>Download</a>&nbsp;&nbsp;<button class='btn btn-xs btn-danger btn-test-delete'  type='button' module='bloodpatho_cert' ref='".$response->id."'>Delete</button></p>";
             $content .="</div>";
            }
              $content .="</div>";
                 $content .="<div class='form-group'>";
            if(!empty($custom_certificates)){
             $content .="<hr/><div class='col-md-12'><label for='' class='control-label'>Custom Certificates</label></div>";
              foreach($custom_certificates as $custom_cert){
                 $content .="<div class='col-md-6'>";
              $content .= "<p>&bull;&nbsp;<a href='".base_url()."../".$custom_cert->cert_file_path."' target='_blank'>".$custom_cert->cert_name."</a>&nbsp;&nbsp;<a class='btn btn-xs btn-primary' target='_blank' href='".base_url()."../".$custom_cert->cert_file_path."' download>Download</a>&nbsp;&nbsp;<button class='btn btn-xs btn-danger btn-custom-test-delete'  type='button' module='custom_cert' ref='".$custom_cert->id."'>Delete</button></p>";
             $content .="</div>";
              }
            }
             $content .="</div>";
    ...
    $content .= "<div class='box-body'>";
        ...
        $content .= "<label for='name' class='control-label'>Number of training hours: </label>";

        $content .= "<input type='number' step='any'  name='num_hours' value='' class='form-control' required>";

        $content .= "</div>";

        $content .= "<div class='form-group'>";
        $content .= "<label for='regenerate_staff_id' class='control-label'>Staff name (on certificate): </label>";
        $content .= "<select name='regenerate_staff_id' id='regenerate_staff_id' class='form-control' required>";
        $content .= "<option value=''>Select staff</option>";
        $CI =& get_instance();
        if (isset($CI->db)) {
          $staff_rows = $CI->db->query("SELECT staff_id, staff_name FROM tbl_staff WHERE status='active' ORDER BY staff_name ASC")->result();
          foreach ($staff_rows as $staff_row) {
            $sid = htmlspecialchars($staff_row->staff_id, ENT_QUOTES, 'UTF-8');
            $sname = htmlspecialchars(ucwords(strtolower($staff_row->staff_name)), ENT_QUOTES, 'UTF-8');
            $content .= "<option value='".$sid."'>".$sname."</option>";
          }
        }
        $content .= "</select>";
        $content .= "<p class='help-block' style='margin-top:6px;font-size:12px;'>Choose who should appear in the BY line. Saved to this certificate when you regenerate.</p>";
        $content .= "</div>";

        $content .= "<script>
        (function(){
          var staffByTest = ".json_encode(array(
            'waive' => isset($response->waived_staff_id) ? (string) $response->waived_staff_id : '',
            'hepab' => isset($response->hepa_staff_id) ? (string) $response->hepa_staff_id : '',
            'harass' => isset($response->harass_staff_id) ? (string) $response->harass_staff_id : '',
            'handhygiene' => isset($response->hygiene_staff_id) ? (string) $response->hygiene_staff_id : '',
            'glucometer' => isset($response->glucometer_staff_id) ? (string) $response->glucometer_staff_id : '',
            'emergency' => isset($response->emergency_staff_id) ? (string) $response->emergency_staff_id : '',
            'bloodpatho' => isset($response->bloodpatho_staff_id) ? (string) $response->bloodpatho_staff_id : '',
            'covid' => isset($response->covid_staff_id) ? (string) $response->covid_staff_id : '',
          )).";
          function syncRegenerateStaff(){
            var t = document.querySelector('select[name=test_type]');
            var s = document.getElementById('regenerate_staff_id');
            if (!t || !s) return;
            var id = staffByTest[t.value] || '';
            if (id) { s.value = id; }
          }
          document.addEventListener('DOMContentLoaded', function(){
            var t = document.querySelector('select[name=test_type]');
            if (t) {
              t.addEventListener('change', syncRegenerateStaff);
              syncRegenerateStaff();
            }
          });
        })();
        ...
        if (isset($get['e']) && $get['e'] === 'staff') {
          $content .= "<div class='alert alert-danger'>Staff could not be resolved. Select a staff name and try again.</div>";
        }
