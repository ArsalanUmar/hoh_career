...
use PHPMailer\PHPMailer\SMTP;
  ...
  function jo_project_path($relative_path = '') {
      $relative_path = str_replace('\\', '/', ltrim((string) $relative_path, '/'));
      if ($relative_path === '') {
          return __DIR__;
      }
      return __DIR__ . DIRECTORY_SEPARATOR . str_replace('/', DIRECTORY_SEPARATOR, $relative_path);
  }

  function jo_resolve_job_application_pdf_path($record, $fallback_relative) {
      $candidates = array();
      if (!empty($record['pdf_file_path'])) {
          $candidates[] = str_replace('\\', '/', $record['pdf_file_path']);
      }
      if (!empty($fallback_relative)) {
          $candidates[] = str_replace('\\', '/', $fallback_relative);
      }
      foreach ($candidates as $rel) {
          if (is_readable(jo_project_path($rel))) {
              return ltrim($rel, '/');
          }
      }
      if (!empty($record['pdf_file_path'])) {
          return ltrim(str_replace('\\', '/', $record['pdf_file_path']), '/');
      }
      return ltrim(str_replace('\\', '/', (string) $fallback_relative), '/');
  }

  function jo_signature_image_path($src) {
      ...
      $abs = jo_project_path($src);
      return is_readable($abs) ? $abs : $src;
  }

  function jo_signature_image_type($src) {
      if (!is_string($src) || $src === '') {
          return '';
      }
      if (strpos($src, 'data:image/') === 0) {
          if (stripos($src, 'image/jpeg') !== false || stripos($src, 'image/jpg') !== false) {
              return 'JPG';
          }
          if (stripos($src, 'image/gif') !== false) {
              return 'GIF';
          }
          return 'PNG';
      }
      $path = jo_signature_image_path($src);
  ...
  function jo_pdf_place_signature($pdf, $src, $x, $y, $w, $h = 0) {
      $image = jo_signature_image_path($src);
      if ($image === '') {
          return false;
      }
      $type = jo_signature_image_type($src);
      if ($type === '') {
          return false;
      }
      try {
          $pdf->Image($image, $x, $y, $w, $h, $type);
          return true;
      } catch (Exception $e) {
          error_log('JO PDF signature skipped: ' . $e->getMessage());
          return false;
      }
  }
  ...
  // echo $ref_decode;
    ...
    if(!empty($_POST)){
      ...
      if($organization == 'Both'){
            ...
            // }
              ...
              $jo_pdf_file = 'pdf/jo_registered_nurse.pdf';
                   $jo_pdf_file2 = 'pdf/jo_hc_registered_nurse.pdf';
     ...
     // die();
        ...
        $pdf_file_name = "pdf_files/".$file_name_pdf.' '.date('Y-m-d',strtotime($_POST['date_filled'])).'.pdf';
        ...
        // if($response){
         ...
         // save_pdf($html,$mySignature,"signatures/".$file_name2,"signatures/".$file_name3,$file_name_pdf,$pdf_consent_file,$post);
          // Regenerate job application PDF so edits appear in "View Full Job Application".
          $pdf_output_path = jo_resolve_job_application_pdf_path($record_details, $pdf_file_name);
          $existing_consent_path = isset($record_details['legal_consent_path']) ? $record_details['legal_consent_path'] : '';
          $consent_output_path = (!empty($existing_consent_path)) ? $existing_consent_path : $pdf_consent_file;
          if (!empty($consent_output_path) && !is_readable(jo_project_path($consent_output_path))) {
              $consent_output_path = $pdf_consent_file;
          }

          save_pdf(
              $html,
              $record_details['signature_path'],
              $record_details['signature_path'],
              $record_details['signature_path'],
              $file_name_pdf,
              $consent_output_path,
          ...
          $con->query("UPDATE `tbl_job_applications` SET `pdf_file_path`='".$con->real_escape_string($pdf_file_path)."' WHERE id='".$ref_decode."'");

   if(!empty($record_details['jo_file_path']) && !empty($jo_pdf_file)){
       ...
       $pdf_jo_file_name = "JO_".$first_name." ".$last_name.' '.date('Y-m-d',strtotime($_POST['date_filled'])).'.pdf';
       $pdf_jo_file_name_2 =  $pdf_jo_file_name_3 =  $pdf_jo_file_name_4 = "";
       if(!empty($jo_pdf_file_2)){

       $pdf_jo_file_name_2 = "JO_2_".$first_name." ".$last_name.' '.date('Y-m-d',strtotime($_POST['date_filled'])).'.pdf';
       }

       if(!empty($jo_pdf_file_3)){

       $pdf_jo_file_name_3 = "JO_3_".$first_name." ".$last_name.' '.date('Y-m-d',strtotime($_POST['date_filled'])).'.pdf';
       }

       if(!empty($jo_pdf_file_4)){

       $pdf_jo_file_name_4 = "JO_4_".$first_name." ".$last_name.' '.date('Y-m-d',strtotime($_POST['date_filled'])).'.pdf';
       }

       $pdf_agr_file_name = "pdf_files/AGR_".$file_name_pdf.' '.date('Y-m-d',strtotime($_POST['date_filled'])).'.pdf';
         ...
         // Preserve existing file paths or generate new ones if they don't exist
         $existing_hh_path = $record_details['hh_file_path'];
         if(!empty($existing_hh_path) && file_exists($existing_hh_path)){
             $file_name_pdf = $existing_hh_path;
         } else {
        ...
        $file_name_pdf_2 = $file_name_pdf_3 = NULL;
         ...
         $query = "UPDATE `tbl_job_applications` SET hh_file_path='".$file_name_pdf."' WHERE id='".$ref_decode."'";
          ...
          $post['name'] = ucwords($first_name." ".$last_name);
           ...
           save_pdf2($html,$record_details['signature_path'], $record_details['signature_path'], $record_details['signature_path'],$pdf_jo_file_name,$pdf_jo_file_name_2 ,$pdf_jo_file_name_3, $pdf_jo_file_name_4,$pdf_agr_file_name,$jo_pdf_file,$jo_pdf_file_2,$jo_pdf_file_3,$jo_pdf_file_4,$file_name_pdf,$file_name_pdf_2,$file_name_pdf_3,$post);
   }
...
Admin
 <br><br>";
  ...
  function save_pdf($html,$signature1,$signature2,$signature3,$name="",$pdf_consent_file="",$post=array(),$output_pdf_path=""){

    $pdf_file = "pdf/job_application_template.pdf";
    $pdf_file_cons = "pdf/legal_consent_template.pdf";
    $date=date('Y-m-d',strtotime($post['date_filled']));
...
// echo $signature1;die();
    ...
    $pdf->SetXY(34 ,121);  
    $pdf->Write(0, ucwords($post['first_name']." ".$post['last_name']));
    jo_pdf_place_signature($pdf, $signature2, 25, 95, 100, 40);
...
// echo $pdf_consent_file;die();
    $pdf->Output(jo_project_path($pdf_consent_file),'F');//exit;
    ...
    $pdf_file_name = !empty($output_pdf_path)
        ? jo_project_path($output_pdf_path)
        : jo_project_path("pdf_files/".$name.' '.$date.'.pdf');
       // echo "<pre>",print_r($post),"</pre>";die();
    ...
    // $pdf->Output($pdf_file_name,'I');exit;
        ...
        $work_duties_1 = str_replace("\r\n", ' ', $post['work_duties_1']);
        $work_duties_2 = str_replace("\r\n", ' ', $post['work_duties_2']);
        $work_duties_3 = str_replace("\r\n", ' ', $post['work_duties_3']);
        
        if (strlen($work_duties_1) > 100 || strlen($work_duties_2) > 100 || strlen($work_duties_3) > 100) {
    $pdf->AddPage(); 
    $pdf->SetFont('Arial','',10); 
    $next_yy = 30;

    if (strlen($work_duties_1) > 100) {
        $pdf->SetXY(28, $next_yy); 
        $pdf->MultiCell(175, 3, $post['work_company_name_1'] . ': ' . $work_duties_1, 0, 'L');
        $next_yy = 80;
    }

    if (strlen($work_duties_2) > 100) {
        $pdf->SetXY(28, $next_yy); 
        $pdf->MultiCell(175, 3, $post['work_company_name_2'] . ': ' . $work_duties_2, 0, 'L');
        $next_yy = 150;
    }

    if (strlen($work_duties_3) > 100) {
        $pdf->SetXY(28, $next_yy); 
        $pdf->MultiCell(175, 3, $post['work_company_name_3'] . ': ' . $work_duties_3, 0, 'L');
    }
}
         // import page 1  
         $pdf->AddPage();  
        $tplIdx = $pdf->importPage(2);  
            jo_pdf_place_signature($pdf, $placed_signature, 35, 175, 100, 40);
                 $pdf->SetFont('Arial','',8);  
             $pdf->SetXY(50 ,202); 
           $pdf->Write(0, $post['first_name']." ".$post['last_name']." / (".$post[
            'position'].")");
        $pdf->SetFont('Arial','',10);  
        $pdf->SetXY(134 ,202);  
         $pdf->Write(0, $date_label);
        ...
        $pdf->useTemplate($tplIdx, 10, 10, 200);  
           ...
           $pdf->Write(0, $post['prof_telno_1']);
            jo_pdf_place_signature($pdf, $placed_signature, 50, 70, 80, 25);
            $pdf->SetXY(135 ,90); 
           $pdf->Write(0,$date_label);
            $pdf->SetFont('Arial','',8);  
                $pdf->SetXY(57 ,89); 
  ...
  $pdf->SetFont('Arial','',10);  
    ...
    $pdf->SetTextColor(0,0,0);
       ...
       // echo "<pre>",print_r($post),"</pre>";die();
    $pdf->SetFont('zapfdingbats', '', 12);
  if(isset($post['hepa']) && ($post['hepa']=="agree"|| $post['hepa']=="yes")){
      $pdf->SetXY(21 ,161.5);  
          $pdf->Write(0, "4");
  }else{
       $pdf->SetXY(21 ,208.5);  
    ...
    $pdf->SetXY(32 ,245);  
    $pdf->Write(0, ucwords($post['first_name']." ".$post['last_name']));
                jo_pdf_place_signature($pdf, $placed_signature, 145, 230, 60, 20);
     $pdf->SetXY(108,245);  
    $pdf->Write(0, $date_label);
    // echo $signature3;die();

    $hbv_pdf_name = !empty($output_pdf_path)
        ? jo_project_path(str_replace('pdf_files/', 'pdf_files/HBV_', $output_pdf_path))
        : jo_project_path("pdf_files/HBV_".$name.' '.$post['date_filled'].'.pdf');
    $pdf->Output($hbv_pdf_name,'F');//exit;
...
// echo "adsa";die();
     // echo $html;die(); // Removed debug statement that was blocking execution

    // Check if jo_pdf_file is empty before proceeding
    if(empty($jo_pdf_file)){
        return; // Exit early if no PDF file is specified
    }
        ...
        for($x=1;$x<=$get_dimensions['num_pages'];$x++){
             ...
             $tplIdx = $pdf->importPage($x);  
              ...
              if($x === $get_dimensions['num_pages']){
                ...
                $pdf->SetXY(30,$get_dimensions['label_y']);  
                $pdf->Write(0, ucwords($post['first_name']." ".$post['last_name']." / (".$post['position'].")"));
              }
        }
