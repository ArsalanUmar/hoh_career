...
// echo "<pre>",  print_r($_POST),"</pre>";die();
    unset($record_details['date_signed']);
    // $_POST= $record_details;
    if($record_details['accept'] == '1'){

    $_POST['i_consent'] = "yes";
  }else{
    $_POST['i_consent'] = 'no';
  }
    $_POST['date'] =  $_POST['date_signed'];// $record_details['date_signed'];
    $_POST['reason'] = $record_details['reason'];
    $_POST['other_reason'] = $record_details['other_reason'];
        $_POST['decline_reason'] = $record_details['reason'];
     ...
     // die();
        ...
        sleep(4);
          ...
          $query = "UPDATE `tbl_influenza_forms` set date_signed='".$post['date_signed']."',  pdf_path = '".$pdf_file_name."'  WHERE  id=".$post['ref'];
          // echo $query;die();
         ...
         save_pdf($html,$signature1,$signature1,$pdf_file_name,$post);
          ...
          // if($send_mail){
            header("Location: influenza_update_form.php?s=1");exit;
...
// echo "<pre>",print_r($post),"</pre>";die();
     ...
     $date_label=date('m/d/Y',strtotime($post['date_signed']));
     if(!empty($post['vaccination_date'])){

      $vaccine_date=date('m/d/Y',strtotime($post['vaccination_date']));
     }else{
      $vaccine_date= NULL;
     }
    $path = "signatures/";
      ...
      $pdf->SetFont('zapfdingbats', '', 13);
          ...
          if($post['i_consent'] != "no"){
            ...
            $pdf->Write(0, "4");
               ...
               if($post['decline_reason']=="I have already received influenza vaccine and have provided documentation"){
                 $pdf->SetXY(27 ,179);   
                $pdf->Write(0, "4");
            }else if($post['decline_reason']=="I believe I will get the flu if I get the shot."){

               $pdf->SetXY(27 ,184);   
               $pdf->Write(0, "4");
            }else if($post['decline_reason']=="I do not like needles."){

               $pdf->SetXY(27 ,188);   
               $pdf->Write(0, "4");
            }else if($post['decline_reason']=="My philosophical or religious beliefs prohibit vaccination."){

               $pdf->SetXY(27 ,193);   
               $pdf->Write(0, "4");
            }else if($post['decline_reason']== "I have a medical contraindication to receiving the vaccine."){
            ...
            }else if($post['decline_reason']=="I do not wish to say why I decline."){
