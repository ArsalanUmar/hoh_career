...
use PHPMailer\PHPMailer\SMTP;
  ...
  use \setasign\Fpdi\Fpdi;
    ...
    $db = new Database;
    $conn = $db->Conn();
    $sql = "SELECT * FROM `tbl_job_applications` where id='".base64_decode($ref)."' ";
    // echo $sql;die();
    ...
    $html ="";
      ...
      $timestamp= date('YmdHis');
       $name=       $file_name_pdf =  $record_details['first_name']." ".$record_details['last_name'];

      $resume_name=$pdf_file_name = "";
     
        $pdf_consent_file = 'pdf_files/LEG_'.$record_details['first_name']." ".$record_details['last_name'].".pdf";

      $query = "UPDATE `tbl_job_applications` SET `legal_consent_path` = '".$pdf_consent_file."' where id='".base64_decode($ref)."'";
          // echo $query;die();
      $result = $conn->query($query);
      $pdf_file_cons = "pdf/legal_consent_template.pdf";
       $date_label=date('F d, Y',strtotime($record_details['date_filled']));
      ...
      $pdf->SetXY(37 ,57);  
      $pdf->Write(0, ucwords($record_details['first_name']." ".$record_details['last_name']));
      $pdf->SetFont('Arial','',10);  
